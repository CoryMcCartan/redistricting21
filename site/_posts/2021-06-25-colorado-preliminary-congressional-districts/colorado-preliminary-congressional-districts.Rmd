...
We also look at what's called a seats-votes curve. This tries to capture
the difficult-to-measure relationship between the number of seats, which
is only 8, and the number of votes, which is orders-of-magnitude larger.
In general, the seats-votes curve sits in the middle of the sampled
plans. That is indicative of a map that was drawn without strongly
considering one party over the other.

<aside>

This idea speaks to the idea of a [partisan bias in this
relationship](https://gking.harvard.edu/files/LP9.pdf). The simulations
of other possible maps helps us understand the geographic relationship
that this measure doesn't use.
...
As for the Mean-Median measure, which is intended to measure partisan
bias, we find that it is slightly to the *left* of the median plan. It's
near the middle, but a bit to the left. Without a focus on
competitiveness, it appears the commission settled into a competitive
plan, relative to the set of plans that approximate the state's
requirements.
