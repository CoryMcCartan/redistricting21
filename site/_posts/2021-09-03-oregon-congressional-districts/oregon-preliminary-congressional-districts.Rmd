...
p[[1]] <- plot_cds(map, map$cd_a, county, 'OR') + labs(title = 'Plan A (Democratic)')
for (i in 2:8) {
  p[[i]] <- plot_cds(map, get_plans_matrix(plans$sb)[, i - 1], county, 'OR') + labs(title = )
}
p[[9]] <- plot_cds(map, map$cd_b, county, 'OR') + labs(title = 'Plan B (Republican)')
```

Using the middle plan, we can compare it to
