...
```{r prop-repr,  fig.width=6, fig.height=3.5, fig.cap="Proportionality and representativeness for the redistricting plans."}
x = as_tibble(plans$tol_01) %>%
    group_by(draw) %>%
    summarize(repr = represent[1],
              prop = proportion[1],
              dem = sum(dem > 0.5)) %>%
ggplot(aes(- abs(prop), repr, color=as.factor(dem))) +
    geom_point(size=0.6, alpha=0.5) +
    scale_color_wa_d("diablo", name="Dem. seats",
                     guide=guide_legend(override.aes=list(size=6, alpha=1, shape=15))) +
    labs(x="Proportionality", y="Representativeness") +
    theme_r21() +
